...
func NewServerAPI(spec *loads.Document) *ServerAPI {
	return &ServerAPI{
		...
		BuyCandyHandler: BuyCandyHandlerFunc(func(params BuyCandyParams) middleware.Responder {
			...
			price:= get_price(*params.Order.CandyType)
			change := *params.Order.Money - ((*params.Order.CandyCount)*price)
			if *params.Order.CandyCount<0 || price==0 {
				bad:=NewBuyCandyBadRequest()
				bad.SetPayload(&BuyCandyBadRequestBody{chek_error(price)})
				return middleware.ResponderFunc(bad.WriteResponse)
			} else if change>=0 {
				creat:=NewBuyCandyCreated()
				res:=C.ask_cow(C.CString("Thank you!"))
				creat.SetPayload(&BuyCandyCreatedBody{change, C.GoString(res)})
				return middleware.ResponderFunc(creat.WriteResponse)
			} else {
				payment:=NewBuyCandyPaymentRequired()
				payment.SetPayload(&BuyCandyPaymentRequiredBody{fmt.Sprintf("You need " + strconv.Itoa(int(change)*-1) + " more money!")})
				return middleware.ResponderFunc(payment.WriteResponse)
			}
		}),
	}
}

func chek_error (a int64) string{
	if a==0 {
return "There are no such candies"
	} 
	return "A negative number of candies has been introduced"
}

func get_price(s string) int64 {
	switch s {
	case "CE":
		return 10
	case "AA":
